...
void applyMotionProfile(uint8_t profileIndex)
{
  ...
  case PROFILE_NERVOUS_IDX:
    p = &PROFILE_NERVOUS;
    break;
  case PROFILE_VERY_NERVOUS_IDX:
    p = &PROFILE_VERY_NERVOUS;
    break;
  default:
    p = &PROFILE_SOFT;
    break;
...
void parseData()
{
  ...
  if (profRaw <= 0)
    ...
    requestedProfile = PROFILE_VERY_NERVOUS_IDX;
  else
  ...
  else if (dynRaw == 2)
  {
    ...
    for (uint8_t m = 0; m < NBMOTEURS; ++m)
    {
      uint8_t idx = 10 + m;
      if (idx < NBDATA)
        changementDeDYNAMIQUE_perMotor[m] = (ABC[idx] != 0);
      else
        changementDeDYNAMIQUE_perMotor[m] = false;
      dynAutoEnabled[m] = false;
    }
  ...
  if (requestedProfile != currentProfile)
  {
    ...
    for (uint8_t i = 0; i < NBMOTEURS; i++)
    {
      if (vUsed[i] < MOTOR_HEUR_V_MIN[i])
        vUsed[i] = MOTOR_HEUR_V_MIN[i];
      if (aUsed[i] < MOTOR_HEUR_A_MIN[i])
  ...
  if (dynRaw == DYN_MODE_AUTO)
  {
    ...
    for (uint8_t i = 0; i < NBMOTEURS; ++i)
    {
      ...
      if (mean < 200.0f)
        enable = true;
      else if (mean > 3000.0f)
        enable = true;
      else if (score > AUTO_ENABLE_SCORE)
        enable = true;
      else
        enable = false;
...
void loop()
{
  ...
  for (uint8_t i = 0; i < NBMOTEURS; i++)
  {
    ...
    */
    ...
    if (emergencyActive)
    {
      ...
      if (emergencyA_target < 50.0f)
        emergencyA_target = 50.0f;
      if (emergencyA_target > ACC_HARD)
        emergencyA_target = ACC_HARD;
      ...
      if (vUsed[i] > VMAX_HARD)
        vUsed[i] = VMAX_HARD;
      if (vUsed[i] < EMERGENCY_V_TARGET)
        vUsed[i] = EMERGENCY_V_TARGET;
      if (aUsed[i] > ACC_HARD)
        aUsed[i] = ACC_HARD;
      if (aUsed[i] < 10.0f)
        aUsed[i] = 10.0f;
    ...
    {
      vTarget = vStream;
      if (aTarget < ACC_MIN_SOFT)
        aTarget = ACC_MIN_SOFT;
    }
    ...
    if (inFlip[i])
    {
      ...
      else
      {
        ...
        if (vTarget < VMIN_USEFUL)
          vTarget = VMIN_USEFUL;
        if (aTarget < ACC_MIN_USEFUL)
          aTarget = ACC_MIN_USEFUL;
      }
    }
  ...
  if (nowMs2 - lastInMs >= PRINT_INTERVAL_MS * 23)
  {
    lastInMs = nowMs2;
    maybeSendINSerial();
  }
  if (nowMs2 - lastOutMs >= PRINT_INTERVAL_MS * 1)
  {
    lastOutMs = nowMs2;
    maybeSendOUTSerial();
  }
  if (nowMs2 - lastDistMs >= PRINT_INTERVAL_MS * 8)
  {
    lastDistMs = nowMs2;
    maybeSendDISTSerial();
  }
